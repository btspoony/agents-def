...
packages: root           # optional; comma list of root | cli | opencode | engine | dsh | omp. Default: root.
---
...
- **`category`** drives the `### <Category>` header under the version section.
  Defaults: `root` → `Harness`, `cli` → `Changed`, `opencode` → `Bundled harness skills (\`harness-skills/\` at publish)`, `engine` → `Changed`, `dsh` → `Changed`, `omp` → `Changed`.
- **`packages`** routes bullets to changelog(s). `root` → root EN + root CN.
  `cli`/`opencode`/`engine`/`dsh`/`omp` → that package's changelog (EN only).
