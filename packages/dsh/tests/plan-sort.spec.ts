 ...
 * 00000810-panel-canvas-zones) reuses the same rule — this module is the
...
describe('plan-sort (spec panel-zones §3)', () => {
  ...
  it("doneAt missing (null / '' / garbage) → empty key sorts LAST, id tie-breaks still apply", () => {
    const plans: PlanLike[] = [
      { id: '00000808-pkg', doneAt: '2026-08-08' }, // valid doneAt wins
    ...
    expect(ids(sortPlans(plans))).toEqual(['00000808-pkg', 'plan-z', 'plan-y', 'plan-x'])
  })

  it('doneAt digitized beats the id-date fallback (fallback only applies among equal doneAt keys)', () => {
    const plans: PlanLike[] = [
      { id: '00000815-no-doneat', doneAt: null },  // id-date 00000815 but no doneAt
      { id: 'plan-old', doneAt: '2026-08-08' },    // doneKey 00000808
    ]
    // Level 1 (doneAt) is authoritative: a plan WITH doneAt always leads.
    expect(ids(sortPlans(plans))).toEqual(['plan-old', '00000815-no-doneat'])
  })

  it('id first-8-digits fallback (/^\d{8}/) DESC when doneAt keys are equal', () => {
    const plans: PlanLike[] = [
      { id: '00000810-some-plan', doneAt: null },
      { id: '00000808-other-plan', doneAt: null },
      { id: 'plan-legacy', doneAt: null }, // no id-date → last
    ]
    expect(ids(sortPlans(plans))).toEqual(['00000810-some-plan', '00000808-other-plan', 'plan-legacy'])
  })

  it('tie-break: identical doneAt + id-date → id lexicographic DESC (deterministic)', () => {
    const plans: PlanLike[] = [
      { id: '00000808-plan-b', doneAt: '2026-08-08' },
      { id: '00000808-plan-a', doneAt: '2026-08-08' },
    ]
    expect(ids(sortPlans(plans))).toEqual(['00000808-plan-b', '00000808-plan-a'])
  })
